classdef GLOBAL < handle
    ...
    properties (SetAccess = ?PROBLEM)
        ...
        DM = 1; %the index of objectives of different decision makers
    end
    ...
    methods
        ...
        function obj = GLOBAL(varargin)
            ...
            propertyStr = {'N', 'M', 'D', 'algorithm', 'problem', 'evaluation', 'run', 'save', 'outputFcn', 'rate'};
        ...
        function Start(obj)
            ...
            if obj.evaluated <= 0
                ...
                obj.Opt = obj.problem.GetOptimum();
        ...
        function notermination = NotTermination(obj, Population)
            ...
            if obj.evaluated >= index * obj.evaluation / num
                [~, next] = unique(Population.decs, 'rows');
                ...
                tmp = inf(length(obj.DM), size(objs, 1));

                for i = 1:length(obj.DM)
                    tmp(i, :) = NDSort(objs(:, obj.DM{i}), size(objs, 1));
                end
                ...
                obj.score(index, :) = {obj.evaluated, Score{1}(1), size(Population, 2)};
            end
