    ...
    for f = 1:length(Fronts)
        ...
        for i = 1:M
            ...
            for j = 2:length(Front) - 1
                c = PopObj(Front(Rank(j + 1)), i) - PopObj(Front(Rank(j - 1)), i);
                dl = (PopObj(Front(Rank(j + 1)), i) - PopObj(Front(Rank(j)), i));
                du = (PopObj(Front(Rank(j)), i) - PopObj(Front(Rank(j - 1)), i));
                pl = dl / c;
                pu = du / c;
                CE =- (dl * log2(pl) + du * log2(pu)) / (Fmax(i) - Fmin(i));
